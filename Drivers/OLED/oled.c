 ...
 * If you want to use hardware I2C, you can comment these.
 */
#define SI2C_Start() SI2C_Start(SSD1306_GPIOx, SSD1306_SCL, SSD1306_SDA)
#define SI2C_Stop() SI2C_Stop(SSD1306_GPIOx, SSD1306_SCL, SSD1306_SDA)
#define SI2C_TransmitByte(BYTE) SI2C_TransmitByte(SSD1306_GPIOx, SSD1306_SCL, SSD1306_SDA, BYTE)
...
void SSD1306_SWriteCommand(uint8_t Command) {
	SI2C_Start();
	SI2C_TransmitByte(SSD1306_Slave_Address);	// slave address
	SI2C_TransmitByte(SSD1306_Control_Command); // write command
	SI2C_TransmitByte(Command);
	SI2C_Stop();
}
...
void SSD1306_SWriteData(uint8_t Data) {
	...
	SI2C_Start();
	SI2C_TransmitByte(SSD1306_Slave_Address); // slave address
	SI2C_TransmitByte(SSD1306_Control_Data);  // write data
	SI2C_TransmitByte(Data);
	SI2C_Stop();
}

void SSD1306_WriteCommand(uint8_t Command) {
	uint8_t data_buffer[2] = { SSD1306_Control_Command, Command };
	HAL_I2C_Master_Transmit(&hi2c1, SSD1306_Slave_Address, data_buffer, 2,
			1000);
}
